    ...
    public class RepairReportController : Controller
    {
        ...
        public IActionResult ExportToExcel(DateTime qtyMonth, string qtyDptId)
        {
            ...
            using (XLWorkbook workbook = new XLWorkbook())
            {
                ...
                ws2.Cell(1, 12).Value = "總工時";
                ws2.Cell(1, 13).Value = "平均每人件數";
                ws2.Cell(1, 14).Value = "維修完工率";
                ws2.Cell(1, 15).Value = "維修(內修)\n3日內完成率";
                ws2.Cell(1, 16).Value = "維修(內修)\n4 - 7日完成率";
                ws2.Cell(1, 17).Value = "維修(內修)\n8日以上完成率";
                ws2.Cell(1, 18).Value = "維修(外修、內外修)\n15日內完成率";
                ws2.Cell(1, 19).Value = "維修(外修、內外修)\n16 - 30日完成率";
                ws2.Cell(1, 20).Value = "維修(外修、內外修)\n31日以上完成率";
                ws2.Cell(1, 21).Value = "增設\n15日內完成率";
                ws2.Cell(1, 22).Value = "增設\n16 - 30日完成率";
                ws2.Cell(1, 23).Value = "增設\n31日以上完成率";
                ws2.Cell(1, 24).Value = "有費用件數";
                ws2.Cell(1, 25).Value = "無費用件數";
                ws2.Cell(1, 26).Value = "總費用";
                ws2.Cell(1, 27).Value = "平均每件\n維修費用";
                ...
                foreach (string l in engs)
                {
                    ...
                    if (ur != null)
                    {
                        ...
                        // 總工時
                        var engHours = qtyEngRepairs.Join(_context.RepairEmps, r => r.repair.DocId, rp => rp.DocId,
                                                            (r, rp) => new
                                                            {
                                                                repair = r,
                                                                repemp = rp
                                                            }).Where(r => r.repemp.UserId == ur.Id)
                                                            .Select(r => new
                                                            {
                                                                hour = r.repemp.Hour,
                                                                minute = r.repemp.Minute
                                                            });
                        var engRepTotalTime = engHours.Select(r => r.hour).Sum() + Decimal.Round(engHours.Select(r => r.minute).Sum() / 60m, 2);
                        // 總花費 & 平均 (已完工案件)(非報廢案件)
